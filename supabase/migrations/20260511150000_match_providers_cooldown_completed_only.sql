-- §4.7 alignment:
-- 1) is_in_cooldown = recent *completed* jobs only (sort last; never exclude).
--    Removes accepted_at from the cooldown flag so we do not over-flag providers
--    or interact badly with tier waves. Supersedes hard-exclude R2 from 20260510160000.
-- 2) App dispatch runs Silver → Bronze → Gold per batch (see lib/orders/dispatchTick.ts).
