-- Provider "ready for next request" while still on an in_progress job.
-- Busy exclusion in match_providers: assigned + en_route always block; in_progress
-- blocks only until ready_for_next_request_at is set (then provider can receive new offers).
-- §4.7: ORDER BY is_in_cooldown only sorts; apply 20260511140000 for completion-based cooldown.

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS ready_for_next_request_at timestamptz;

COMMENT ON COLUMN public.orders.ready_for_next_request_at IS
  'When set on an in_progress order, provider is eligible for new dispatch offers while finishing this job.';
