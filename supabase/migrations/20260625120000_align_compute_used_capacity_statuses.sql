-- Align compute_used_capacity with demand-zone active-order statuses.
-- Pricing RPC previously counted only pending/assigned/in_progress, so orders
-- in offered/en_route/arrived (e.g. while searching) showed 0% capacity.
...
comment on function public.compute_used_capacity(text, text) is
  'Used capacity % per (area, service): in-flight bookings in last 30 min / online providers (spec §2.3).';
