-- Winner cooldown (Architecture / product copy):
-- • Recent win = accepted_at OR completed_at within the last 5 minutes.
-- • Deprioritize only: ORDER BY is_in_cooldown ASC (never exclude here).
-- • Exposes is_in_cooldown on the result set for callers.
-- Silver/Bronze still get waves on schedule; cooldown gold sorts after other golds
-- in the same tier wave so lower tiers are not blocked by a saturated gold list.
--
-- Postgres 42P13: cannot change RETURNS TABLE row type with CREATE OR REPLACE.
-- Drop overloads (8-arg wrapper first — it calls the 11-arg implementation), then recreate.
...
-- Backward-compatible 8-arg entrypoint (default wave params + no tier filter).
CREATE OR REPLACE FUNCTION public.match_providers(
  p_mode_id text,
  p_target_id text,
  p_category_id text,
