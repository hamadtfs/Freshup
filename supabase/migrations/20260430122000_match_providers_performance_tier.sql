-- Add optional performance-tier filter (Architecture §4.3).
-- When p_performance_tier is NULL or '', no tier filter is applied (backward compatible).
...
online_candidates AS (
  ...
  WHERE COALESCE(pd.is_online, false) = true
    AND (
      ...
      OR lower(trim(pd.dispatch_performance_tier)) = (SELECT performance_tier FROM req)
    )
),
provider_points AS (
  SELECT
