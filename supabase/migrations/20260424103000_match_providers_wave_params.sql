-- Milestone 3: allow per-wave distance/rating params in match_providers.
-- Keeps backward compatibility by retaining the original 8-arg signature.

-- If a wave-aware signature already exists with a different OUT row type,
-- CREATE OR REPLACE will fail. Drop it first, then recreate.
DROP FUNCTION IF EXISTS public.match_providers(
  text, text, text, text, text, double precision, double precision, timestamptz, double precision, double precision
);

-- New signature with wave params (max distance + min rating).
...
-- Backward-compatible 8-arg signature (older callers).
DROP FUNCTION IF EXISTS public.match_providers(
