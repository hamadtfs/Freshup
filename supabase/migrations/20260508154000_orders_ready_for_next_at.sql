-- "Ready for next request" — provider opt-in mid-job availability.
--
-- Pricing & Tier System Spec §3.1 / boss feedback on M3 cooldown:
--   "A haircut takes 30–45 minutes, and the next customer needs 10–15
--    minutes to travel — so the provider should be ready around halfway
--    through. The button must let the provider opt back into the
--    dispatch pool while the current job is still in progress."
--
-- Implementation:
--   1. Adds a nullable `ready_for_next_at` timestamptz column to
--      `public.orders`. NULL = same behaviour as before (provider is
--      considered busy until the order is completed). NOT NULL = the
--      provider has explicitly tapped "Ready for next request".
--   2. Updates `match_providers` so the busy-provider exclusion clause
--      ignores in-progress orders whose `ready_for_next_at` is set.
--      Everything else about `match_providers` is unchanged.
--   3. Adds a partial index for the typical query pattern (provider
--      currently has ready-for-next set).
--
-- Safety:
--   - Column is nullable, no default, so existing rows stay NULL.
--   - The SQL change is purely additive (extra `AND` in the existing
--     subquery). When `ready_for_next_at` is NULL the busy exclusion
...
ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS ready_for_next_at timestamptz;

COMMENT ON COLUMN public.orders.ready_for_next_at IS
  'Set by the assigned provider when they tap "Ready for next request" '
  'mid-job. While set, match_providers will not exclude this provider '
  'on the basis of the current order being in progress.';

CREATE INDEX IF NOT EXISTS orders_ready_for_next_idx
  ON public.orders (provider_id, ready_for_next_at)
  WHERE ready_for_next_at IS NOT NULL;

-- ────────────────────────────────────────────────────────────────────
-- 2. match_providers: ignore in-progress orders that are ready-for-next
-- ────────────────────────────────────────────────────────────────────
  ...
  WHERE wr.service_rating >= r.min_rating
    AND NOT EXISTS (
      ...
      WHERE o.provider_id = wr.provider_id
        ...
        -- "Ready for next request": when the provider has explicitly
        -- opted back into the dispatch pool mid-job, do NOT exclude
        -- them on the basis of this order.
